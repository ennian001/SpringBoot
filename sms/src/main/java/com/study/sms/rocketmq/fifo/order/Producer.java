 ...
 * 下面用订单进行分区有序的示例。一个订单的顺序流程是：创建、付款、推送、完成。订单号相同的消息会被先后发送到同一个队列中，消费时，
...
//        producer.setNamesrvAddr("192.168.72.137:9876");
//        producer.setNamesrvAddr("47.105.157.113:9876");
        producer.setRetryTimesWhenSendFailed(6000);
        producer.start();

//        String[] tags = new String[]{"TagA","TahC","TagD"};
        ...
        for (int i = 0; i < 10; i++) {
            //加个时间戳
            String body = dateStr+"Hello RcketMq" + orderList.get(i);
